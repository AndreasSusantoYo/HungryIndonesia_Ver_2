...
<html lang="en">
  ...
  <body class="bg-[#FFFAE5]">
    ...
    <div class="ml-10 mt-10">
      <a href="HomePageLand.html" class="w-8 h-8 inline-block"
    ...
    <div class="mx-auto w-1/3 text-center">
      ...
      <div class="flex justify-center">
        <!-- Login Button -->
        <button
          class="w-1/2 border-2 px-20 py-2 rounded-l-3xl border-r-0 text-[#5F5D2D] bg-[#5f5d2d] text-[#fff] font-bold shadow-inner"
        >
          Log In
        </button>
        <!-- Signup Button -->
        <button
          class="w-1/2 border-2 px-20 py-2 rounded-r-3xl border-l-0 text-[#5F5D2D] font-bold hover:bg-[#c9c8ae] shadow-inner"
    ...
    <!-- Placeholder for displaying login response -->
    ...
    if ($result) {
        if (mysqli_num_rows($result) > 0) {
            ...
            if ($password === $row['password']) {
                ...
                $_SESSION['id'] = $row['id'];
                echo "<script>window.location.href = 'HomePageIn.php';</script>";
